...
const token = process.env.API_FOOTBALL_TOKEN || process.env.API_FOOTBALL_KEY || process.env.API_FUTEBOL_KEY;
...
export default async function handler(request, response) {
  ...
  if (!token) {
    response.status(500).json({
      ...
      message: 'Configure a chave da API nas variaveis de ambiente do Vercel.',
    });
    return;
  }

  const route = String(request.query.route || 'fixtures');
  const cleanRoute = route.replace(/^\/+/, '');
  const targetUrl = new URL(`${baseUrl.replace(/\/$/, '')}/${cleanRoute}`);
